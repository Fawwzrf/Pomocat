...
export function initTimer(elements, initialSettings, onSessionComplete) {
    ...
    function startTimer() { if (timerInterval) return; isPaused = false; startPauseBtn.textContent = 'Pause'; timerInterval = setInterval(() => { timeLeft--; updateDisplay(); if (timeLeft <= 0) { clearInterval(timerInterval); timerInterval = null; if (alarmSound) { alarmSound.play().catch(e => console.error("Gagal memutar suara:", e)); } if (currentMode === 'pomodoro') { pomodoroCycle++; if (settings.autoCheckTasks) { onSessionComplete(); } showNotification("Sesi fokus selesai! Waktunya istirahat."); const nextMode = pomodoroCycle % settings.longBreakInterval === 0 ? 'longbreak' : 'shortbreak'; setMode(nextMode); if (settings.autoStartBreaks) setTimeout(() => startTimer(), 1000); } else { showNotification("Istirahat selesai! Waktunya kembali fokus."); setMode('pomodoro'); if (settings.autoStartPomodoros) setTimeout(() => startTimer(), 1000); } } }, 1000); }
